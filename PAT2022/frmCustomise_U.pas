...
uses
  ...
  Vcl.ComCtrls, clsFormatCalculations, Vcl.Buttons, VCLTee.TeCanvas;
...
uses
  frmWelcome_U, frmCourseBrowser_U;
...
begin
  ...
  begin
    ShowMessage('Select a Preset.');
    exit;
  end
  else if lstPresets.ItemIndex = 0 then
  begin
    reset(tColour);
    rewrite(tPreset1);
  ...
  // Could do a case but doesn't really matter
end;
